	...
	if ( is_orig )
		...
		if ( name == "USER-AGENT" )
			...
			if (/PA QQ/ in value)
				...
				Software::found(c$id,[$version=result$version,$name=result$name,$host=c$id$resp_h, $o_host=c$id$orig_h, $host_p=c$id$resp_p,$software_type=HTTP::QQ,$unparsed_version=value]);
				}
			...
			if (dynamic_content_pattern in c$http$uri)
				...
				if ( 2 in temp)
					...
					qq = [$unparsed_version=value, $host=c$id$resp_h, $o_host=c$id$orig_h,$host_p=c$id$resp_p, $software_type=QQ];
					qq$QQ_num=temp[0];
